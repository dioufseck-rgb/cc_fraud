...
async def get_chain(instance_id: str) -> list[dict[str, Any]]:
    ...
    Each entry is an enriched InstanceState with:
      - result       (step summaries from instance.result)
    ...
    # Resolve instance to get its correlation_id
    ...
    # Fetch all instances in the chain (DESC by created_at)
    ...
    # Index pending approvals by instance_id
    try:
    ...
    for inst in chain_instances:
        # Work orders sent by this instance
        try:
            work_orders = [
                {
