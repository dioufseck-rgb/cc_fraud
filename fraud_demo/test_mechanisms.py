...
COORD_CONFIG = "fraud_demo/coordinator_config.yaml"
CASE_FILE = "fraud_demo/cases/app_scam_romance.json"
...
def test_tool_registry_from_case_dir():
    ...
    # Without case_dir — should find nothing (files are in fraud_demo/cases/fixtures/)
    ...
    reg_demo = create_case_registry(lean, fixtures_dir="fraud_demo/cases")
    ...
    for case_id, expected_name in [
        ...
        reg = create_case_registry({"case_id": case_id}, fixtures_dir="fraud_demo/cases")
...
def test_delegation_result_injection():
    ...
    config, _ = load_three_layer(
        "fraud_demo/workflows/fraud_specialty_investigation.yaml",
        "fraud_demo/domains/app_scam_fraud.yaml",
    )
    ...
    state_snapshot = {
        ...
        "steps": [
            ...
            {"step_name": "think_determination", "primitive": "think",
             "output": {"decision": "file_sar", "confidence": 0.88}},
        ],
        "delegation_results": {
    ...
    results.append(pass_fail(
        "Prior steps preserved (retrieve, investigate, think)",
        step_names == ["retrieve_evidence", "investigate_activity", "think_determination"],
        f"Got: {step_names}",
    ))
...
def test_full_chain():
    ...
    child_registry = create_case_registry(child_input, fixtures_dir="fraud_demo/cases")
